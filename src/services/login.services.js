...
class LoginService {
    ...
    loginUser = async (userData) => {
        ...
        //aplica la modificacion de la fecha de inicio de sesión
    ...
    userDocuments = async (userId, files) => {
        try {
          const user = await this.login.findUserById(userId);
    
          if (!user) {
            throw new Error('Usuario no encontrado');
          }
    
          // Actualizar el estado del usuario o realizar otras operaciones según tus requisitos
          user.status = 'Documentos cargados';
    
          // Guardar los archivos en la base de datos o realizar otras operaciones según tus requisitos
          ...
          return { message: 'Archivos subidos y usuario actualizado correctamente' };
        } catch (error) {
          throw error;
        }
    };
